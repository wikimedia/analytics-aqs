...
aqsUtil.normalizeResponse = function(res) {
    ...
    const etag = crypto.createHash('sha1').update(JSON.stringify(res.body)).digest('hex');
    res.headers = res.headers || {};
    res.headers['cache-control'] = 's-maxage=14400, max-age=14400';
 ...
 *
 * Mediarequest paths also include some characters, mainly punctuation ones,
 * that are stored in its non-urlencoded form, so we have to handle encoding taking
 * those exceptions into account.
 */
const encodeMediarequestPath = (path) => {
    const charExceptions = new Set('($)=`%*!&+:?"\';@^'.split(''));
    const encodedPath =  path.split('').reduce((resultingPath, char) => {
        if (charExceptions.has(char)) {
            resultingPath += char;
        } else {
            resultingPath += encodeURIComponent(char);
        }
...
aqsUtil.normalizeFileURI = (fileURI) => {
    ...
    const normalizedPath = pathBeforeFileName + encodeMediarequestPath(fileName);
...
/**
 * Removes countries listed in sys/geoeditors-country-deny-list.js
 * from the provided array of by country data.
 *
 *
 * Returns
 *  The list of countries in the same format as given, with unallowed
 *  countries removed.
 *
 */
aqsUtil.purgeDeniedCountries = function(countries) {
    return countries
        .filter(countryJSON => !countryDenyList.includes(countryJSON.country));
